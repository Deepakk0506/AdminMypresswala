-- Insert Dummy Data for Additional Customer Tables
...
SELECT 
    ...
    s.id as service_id,
    o.id as order_id,
    FLOOR(random() * 5) + 1 as rating_value,
    CASE 
...
AND random() < 0.7; -- 70% of customers leave reviews
...
SELECT 
    ...
    title as title,
    CASE 
        WHEN title = 'Order Confirmation' THEN 'Your order #' || FLOOR(random() * 10000 + 1000) || ' has been confirmed and is being processed.'
        WHEN title = 'Delivery Update' THEN 'Your order is out for delivery. Expected arrival: 2-3 hours.'
        WHEN title = 'Payment Received' THEN 'Thank you! Your payment of ₹' || (FLOOR(random() * 5000 + 500) || ' has been received.'
        WHEN title = 'Special Offer' THEN 'Get 20% off on your next laundry service! Use code SAVE20.'
        ELSE 'Welcome to our service! We are excited to serve you.'
    END as message,
    CASE 
        WHEN title IN ('Order Confirmation', 'Delivery Update', 'Payment Received') THEN 'order_update'
        WHEN title = 'Special Offer' THEN 'promotion'
        ELSE 'system'
    END as type,
    random() < 0.6 as is_read,
    CASE 
        WHEN title LIKE '%order%' THEN jsonb_build_object('order_id', FLOOR(random() * 10000 + 1000))
        WHEN title = 'Special Offer' THEN jsonb_build_object('discount_code', 'SAVE20', 'discount_percent', 20)
        ELSE '{}'::jsonb
    END as data,
...
-- 5. Add some multiple addresses per customer
INSERT INTO customer_addresses (customer_id, address_type, address_line1, address_line2, city, state, postal_code, is_default, created_at)
SELECT 
    id as customer_id,
    ...
    (ARRAY['Mumbai', 'Delhi', 'Bangalore', 'Pune'])[FLOOR(random() * 4)] as city,
    (ARRAY['Maharashtra', 'Delhi', 'Karnataka', 'Maharashtra'])[FLOOR(random() * 4)] as state,
    LPAD(FLOOR(random() * 900000 + 100000)::text, 6, '0') as postal_code,
    false as is_default,
    NOW() as created_at
FROM customers
WHERE id IS NOT NULL
AND random() < 0.4; -- 40% of customers have billing address
...
FROM customers
WHERE id IS NOT NULL
AND random() < 0.5; -- 50% of customers have UPI

-- 7. Add more notifications
INSERT INTO notifications (user_id, title, message, type, is_read, data, created_at)
SELECT 
    id as user_id,
...
FROM customers
WHERE id IS NOT NULL
AND random() < 0.6; -- 60% have service completion notifications
...
-- 9. Show sample data
SELECT 'Sample Customer Addresses:' as info;
SELECT * FROM customer_addresses LIMIT 3;

SELECT 'Sample Payment Methods:' as info;
SELECT * FROM payment_methods LIMIT 3;
